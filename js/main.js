...
let menuBody = document.querySelector(".menu-header__list");
if (iconMenu) {
...
function popupOpen(pl, v) {
	...
	if (!$('.menu-header__list').hasClass('active')) {
...
$('.goto').click(function () {
	...
	if ($('.menu-header__list').hasClass('active')) {
		$('.menu-header__list,.burger').removeClass('active');
		$('body').removeClass('lock');
	}
	return false;
});
...
if ($('.content').length > 0) {
	$(window).scroll(function(){
    	...
    	const mainblockOffset = $('.content').offset().top;
    	if (scrolled > mainblockOffset) {
    		$('.header').addClass('active')
    	} else if (scrolled < mainblockOffset) {
...
let catalogBurger = document.querySelector(".sidebar__icon");
if (catalogBurger) {
	catalogBurger.addEventListener("click", function () {
		catalogBurger.classList.toggle("active");
		$('.sidebar__list').toggle("active");
	});
}

//Класс .active для кнопок меню.
$(function () {  
